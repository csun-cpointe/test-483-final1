...
def projectDirectory = 'test-483-final1'
def jenkinsSteps
// Set URL of ArgoCD server
def argocdUrl = ''
def argocdAppName = 'test-483-final1'
def argocdNamespace = 'argocd'
def argocdDestinationServer = 'https://kubernetes.default.svc'
def argocdBranch = '${branch}'
def argocdSyncLabel = 'argocd.argoproj.io/instance=test-483-final1'
            ...
            try {
                dir(projectDirectory) {
                    slackSend color: "warning",
                            message: "Verify 1.11.0-SNAPSHOT Release authenticating"
                    jenkinsSteps.argocdAuthenticate(argocdUrl, '${argocdPassword}')
                }
            } catch (err) {
                slackSend color: "danger",
                        message: "Verify 1.11.0-SNAPSHOT Release failed to authenticate properly"
                throw err
            }
        }
    }
...
stage("Deploy") {
    node('master') {
        try {
            dir(projectDirectory) {
                slackSend color: "warning",
                        message: "Verify 1.11.0-SNAPSHOT Release deploying"
                ...
                slackSend color: "good",
                        message: "Verify 1.11.0-SNAPSHOT Release deployed successfully"
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "Verify 1.11.0-SNAPSHOT Release failed to deploy properly"
            throw err
        }
    }
}
...
stage("Teardown") {
    node('master') {
        try {
            dir(projectDirectory) {
                slackSend color: "warning",
                        message: "Verify 1.11.0-SNAPSHOT Release shutting down"
                jenkinsSteps.argocdTerminate(argocdAppName)
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "Verify 1.11.0-SNAPSHOT Release failed to shut down properly"
            throw err
        }
    }
}
